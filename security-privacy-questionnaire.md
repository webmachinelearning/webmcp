...
We would like to bring attention to a security risk not directly surfaced by this questionnaire: agents browsing multiple origins may carry state from one origin to another, potentially leading to data leakage or same-origin policy bypasses if not handled securely by the user agent. This risk is inherent to multi-origin agent browsing and exists independently of WebMCP.
